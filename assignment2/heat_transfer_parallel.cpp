...
inline void heat_transfer_calculation(uint tid, uint size, uint start, uint end,
  ...
  for (stepcount = 1; stepcount <= steps; stepcount++) {
    ...
    tBarrier.start();
    barrier->wait();
    barrier_time_taken += tBarrier.stop();
    if (tid == 0) {
  ...
  *time_taken = t1.stop();
  std::cout<<"t"<<tid<< "waited at barrier for: "<<barrier_time_taken<<std::endl;
}
...
int main(int argc, char *argv[]) {
  ...
  if (grid_size <= 0 || n_threads <= 0 || steps <= 0) {
    throw std::invalid_argument(
        "The commandline arguments: --gSize, --nThreads and --tSteps must be "
        "at least 1.\n");
  }
